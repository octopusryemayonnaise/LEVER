...
def build_constraints(G, itr, N, end, M, E):
    ...
    for edge in G.edges:
        variables[edge] = (
            LpVariable(name=f"{edge[0]}_{edge[1]}_l", lowBound=0, cat="Integer"),
            LpVariable(name=f"{edge[0]}_{edge[1]}_u", lowBound=0, cat="Integer"),
        )

    objective_function = lpSum(
        variables[edge][1] - variables[edge][0] for edge in G.edges
    )
    ...
    for edge in G.edges:
        ...
        total = lpSum(
            variables[(s_prime, next_node)][1] for next_node in G.successors(s_prime)
        )
        ...
        if s_prime == end:
            ...
            #   ti_u = total – (p – 1)
            constraints.append(
                variables[edge][1] - (total - (p - 1)) - M * (total_exceeds_N) <= 0
            )
            constraints.append(
                variables[edge][1] - (total - (p - 1)) + M * (total_exceeds_N) >= 0
            )
...
def solve_integer_programming(
    dag, objective_function, constraints, variables, get_dag=True
):
    ...
    if get_dag:
        ...
        for i in range(dag.gridworld.state_count):
            ...
            min_iterations = [
                [0] * dag.gridworld.action_count
                for _ in range(dag.gridworld.state_count)
            ]
            max_iterations = [
                [0] * dag.gridworld.action_count
                for _ in range(dag.gridworld.state_count)
...
def solve_IP(dag, N, start, end, M=100000, E=0.00001, get_dag=True):
    if get_dag:
        ...
        objective_function, constraints, variables = build_constraints(
            dag.graph, itr, N, end, M, E
        )
        return solve_integer_programming(
            dag, objective_function, constraints, variables
        )
    else:
        ...
        return solve_integer_programming(
            dag, objective_function, constraints, variables, get_dag
        )
